    ...
    public static partial class WoW
    {
        ...
        public static int Level
        {
            get
            {
                ...
                try
                {
                    Log.WriteDirectlyToLogFile($"Level Green = {c.G} Blue = {c.B}");
                    if (c.B == 255 && c.B == 255)
                        return 0;
                ...
                {
                    Log.Write($"Level Logic Green = {c.G} Blue = {c.B}");
                    Log.Write(ex.Message, Color.Red);
                }
                return 0;
            }
        ...
        public static void TargetPartyMember(int id)
        {
            if (id <= 0)
        ...
        public static void Initialize(Process wowProcess)
        {
            ...
            if (ConfigFile.ReadValue("CloudMagic", "AddonName") == "")
            {
                Log.Write("This is the first time you have run the program, please specify a name you would like the CloudMagic addon to use");
                Log.Write("this can be anything you like (letters only no numbers)");

                while (ConfigFile.ReadValue("CloudMagic", "AddonName") == "")
                {
            ...
            Log.Write($"Addon Name set to: [{ConfigFile.ReadValue("CloudMagic", "AddonName")}]", Color.Blue);
