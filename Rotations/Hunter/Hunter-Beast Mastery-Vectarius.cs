...
//pet control			
		...
		if (DetectKeyPress.GetKeyState(0x6A) < 0)
            {
                WoW.HasBossTarget = !WoW.HasBossTarget;
                Thread.Sleep(150);
            }			
...
//aspect_of_the_wild,if=buff.bestial_wrath.up|target.time_to_die<12	
					if (WoW.CanCast("Aspect of the Wild")&& WoW.HasBossTarget && WoW.PlayerHasBuff("Bestial Wrath") && WoW.IsSpellInRange("Cobra Shot"))
                    {
...
//aspect_of_the_wild,if=buff.bestial_wrath.up|target.time_to_die<12	
					if (WoW.CanCast("Aspect of the Wild") && WoW.HasBossTarget && WoW.PetHasBuff("Beast Cleave") && WoW.PetBuffTimeRemaining("Beast Cleave") > GCD && WoW.PlayerHasBuff("Bestial Wrath") && WoW.IsSpellInRange("Cobra Shot"))
                    {
                        WoW.CastSpell("Aspect of the Wild");
                        return;
                    }	
