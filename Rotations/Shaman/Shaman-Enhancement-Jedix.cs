        ...
        public override void Pulse()
        {
            ...
            if (combatRoutine.Type == RotationType.SingleTarget) // Do Single Target Stuff here
            {
                if (WoW.HasTarget && WoW.TargetIsEnemy && !WoW.PlayerHasBuff("Ghost Wolf") && WoW.IsInCombat) //First things go first but break if we wanna run in Wolf
                {
					...
					if (WoW.CanCast("Feral Spirit") && WoW.IsSpellInRange("Stormstrike") && combatRoutine.WoW.HasBossTarget && WoW.Maelstrom >= 20) //Wolves in melee range
                    {
					...
					if (WoW.CanCast("Doom Winds") && combatRoutine.WoW.HasBossTarget && WoW.IsSpellInRange("Stormstrike") && WoW.PlayerHasBuff("Flametongue"))
                    {
