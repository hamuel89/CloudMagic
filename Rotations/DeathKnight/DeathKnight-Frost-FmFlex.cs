...
namespace CloudMagic.Rotation
{
    public class DKFrostMGFmflex : CombatRoutine
    {
        ...
        public void BreathRotation()
        {
            ...
            if (isMelee && combatRoutine.WoW.HasBossTarget && isCheckHotkeysFrostOffensivePillarofFrost && !WoW.IsSpellOnCooldown("PillarofFrost") && hasBreath)
            {
                WoW.CastSpell("PillarofFrost");
            }
            if ((haveCoF || useNextHRWCharge) && haveHRW && runicPower <= 30 && isCheckHotkeysFrostOffensiveErW && combatRoutine.WoW.HasBossTarget && !WoW.PlayerHasBuff("HEmpower Rune") && !WoW.IsSpellOnCooldown("HEmpower Rune") && hasBreath)
            {
                useNextHRWCharge = false;
                WoW.CastSpell("HEmpower Rune");
            }
            if ((haveCoF || useNextHRWCharge) && !haveHRW && runicPower <= 50 && currentRunes <=1 && isCheckHotkeysFrostOffensiveErW && combatRoutine.WoW.HasBossTarget && !WoW.IsSpellOnCooldown("HEmpower Rune") && hasBreath)
            {
                WoW.CastSpell("Empower Rune");
            }
            if (combatRoutine.WoW.HasBossTarget && isMelee && currentRunes >= 2 && runicPower >= 70 && CanCastNoRange("Breath"))
            {
            ...
            if (isMelee && currentRunes >= 1 && ((runicPower >= 48 && hasBreath) || !hasBreath) && (!combatRoutine.WoW.HasBossTarget || (combatRoutine.WoW.HasBossTarget && WoW.SpellCooldownTimeRemaining("Breath") >= 15)) && CanCastNoRange("Remorseless Winter"))
            {
            ...
            if (runicPower >= 25 && CanCastInRange("Frost Strike") && !hasBreath && (!combatRoutine.WoW.HasBossTarget || (combatRoutine.WoW.HasBossTarget && WoW.SpellCooldownTimeRemaining("Breath") >= 15)))
            {
                WoW.CastSpell("Frost Strike");
                return;
            }
        ...
        {
            if (isCheckHotkeysFrostOffensivePillarofFrost && isMelee && combatRoutine.WoW.HasBossTarget && !WoW.IsSpellOnCooldown("PillarofFrost"))
            {
                WoW.CastSpell("PillarofFrost");
            }
            if (combatRoutine.WoW.HasBossTarget && isCheckHotkeysFrostOffensiveErW && isMelee && currentRunes == 0 && WoW.PlayerHasBuff("PillarofFrost") && !WoW.IsSpellOnCooldown("Empower Rune"))
            {
                WoW.CastSpell("Empower Rune");
            }
            if (combatRoutine.Type == RotationType.SingleTarget || combatRoutine.Type == RotationType.SingleTargetCleave) // Do Single Target Stuff here
            {
                if (CanCastInRange("Frost Strike") && (!WoW.PlayerHasBuff("Icy Talons") || WoW.PlayerBuffTimeRemaining("Icy Talons") <= 2) && runicPower >= 25 &&
                    !(combatRoutine.WoW.HasBossTarget && CanCastNoRange("Obliteration") && isTalentOblitaration) &&
                ...
                if (combatRoutine.WoW.HasBossTarget && isMelee && currentRunes >= 2 && runicPower >= 25 && isTalentOblitaration && CanCastNoRange("Obliteration"))
                {
                ...
                if (isTalentOblitaration && runicPower >= 40 && CanCastInRange("Frost Strike") && !(combatRoutine.WoW.HasBossTarget && CanCastNoRange("Obliteration")) && isTalentOblitaration &&
