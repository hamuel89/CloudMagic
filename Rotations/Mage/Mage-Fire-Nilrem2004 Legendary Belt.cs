		...
		public override void Pulse()
        {
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.VK_RCONTROL) < 0)
            {
                ...
                { 
                    ...
                    Log.Write("Cooldowns "+ (combatRoutine.WoW.HasBossTarget ? "On" : "Off"));
                    togglewatch.Restart();
					if (!WoW.HasBossTarget && Opener)
					{
						Opener = !Opener;
						Log.Write("Opener "+ (Opener ? "Enabled" : "Disabled"));
						Log.WriteCloudMagic("Disabling Opener because you disabled Cooldowns.", Color.Red);
					}
					if (!WoW.HasBossTarget && autoCD)
					{
						autoCD = !autoCD;
						Log.Write("Auto Burst "+ (Opener ? "Enabled" : "Disabled"));
						Log.WriteCloudMagic("Disabling Automatic burst because you disabled Cooldowns.", Color.Red);
					}
                }
            }
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.VK_F1) < 0)
            {
                ...
                { 
					if (!Opener)
					{
						if (WoW.HasBossTarget && WoW.PlayerSpellCharges("Phoenix") == 3 && WoW.PlayerSpellCharges("Fire Blast") == 3 && !WoW.IsSpellOnCooldown("Combustion")
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.VK_F4) < 0)
            {
                ...
                { 
				    pyrocast = !pyrocast;
                    Log.Write("Using "+ (pyrocast ? "Pyro" : "Flamestrike"));
			...
			if (DetectKeyPress.GetKeyState(DetectKeyPress.Alt) < 0 && WoW.HasBossTarget)
            {
            ...
            if (combatRoutine.Type == RotationType.SingleTarget) 
            {
				...
				if (!CombatWatch.IsRunning && WoW.IsInCombat && WoW.HasBossTarget && Opener && WoW.HasTarget && WoW.TargetIsEnemy)
					{
						CombatWatch.Start();
				...
				// Log.WriteCloudMagic("Force Pyro "+ (ForcePyro ? "TRUE" : "FALSE"), Color.Red);
					...
					&& WoW.TargetHealthPercent <= 30 && !WoW.WasLastCasted("Combustion")) /* What to do if we are NOT MOVING - NON BURST PHASE */
				{
					...
					{ 
						...
						//WoW.CastSpell("Pyroblast");
						Log.WriteCloudMagic("FORCING PYRO/FLAMESTRIKE....", Color.Red);
						ForcePyro = !ForcePyro;
						if (WoW.PlayerSpellCharges("Fire Blast") >= 1 && WoW.HasBossTarget
						...
						else if (WoW.PlayerSpellCharges("Fire Blast") >= 1 && !WoW.HasBossTarget) 
						{
							Thread.Sleep(500);
							WoW.CastSpell("Fire Blast");
							return;
					...
					/* if (!WoW.HasBossTarget && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.IsSpellInRange("Fireball") && WoW.PlayerSpellCharges("Fire Blast") >= 1 
					...
					if (!WoW.HasBossTarget && WoW.IsInCombat && WoW.HasTarget && WoW.TargetIsEnemy && WoW.PlayerSpellCharges("Fire Blast") == 0 && WoW.CanCast("Phoenix")
